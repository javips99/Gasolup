...
const App = (() => {
     ...
     * Usa AbortController para evitar que una respuesta lenta bloquee la UI.
     *
     * @param {string} localidad - Nombre de ciudad o municipio
     */
    async function _geocodificarLocalidad(localidad) {
        // Validar entrada antes de hacer la petición
        const validacion = validarLocalidad(localidad);
        if (!validacion.ok) {
            UIModule.mostrarError(validacion.error);
            return;
        }
        ...
        const timerId = setTimeout(() => controller.abort(), NOMINATIM_TIMEOUT);
        ...
        try {
            ...
            if (error.name === 'AbortError') {
                UIModule.mostrarError('La búsqueda tardó demasiado. Comprueba tu conexión e inténtalo de nuevo.');
            } else {
     ...
     * Comprueba navigator.onLine antes de lanzar la petición para mostrar
     * un mensaje de error específico cuando no hay conexión.
     */
    async function _cargarDatos() {
        if (_cargando || !_coordenadasUsuario) return;

        // Detectar falta de conexión antes de intentar la petición
        if (!navigator.onLine) {
            UIModule.mostrarError('Sin conexión a internet. Conéctate e inténtalo de nuevo.');
            _mostrarToast('Sin conexión a internet', 'error');
            return;
        }
